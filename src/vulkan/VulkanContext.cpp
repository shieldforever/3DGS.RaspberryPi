...
void VulkanContext::createInstance() {
    ...
    vk::ValidationFeatureEnableEXT enables[] = {vk::ValidationFeatureEnableEXT::eDebugPrintf};
    vk::ValidationFeaturesEXT features = {};
    features.enabledValidationFeatureCount = 1;
    features.pEnabledValidationFeatures = enables;

    vk::StructureChain<vk::InstanceCreateInfo, vk::ValidationFeaturesEXT, vk::DebugUtilsMessengerCreateInfoEXT> createInfoChain = {
        {
...
void VulkanContext::createQueryPool() {
    ...
    commandBuffer->resetQueryPool(queryPool.get(), 0, 20);
...
void VulkanContext::createLogicalDevice(vk::PhysicalDeviceFeatures deviceFeatures,
    ...
    // Query supported features to avoid ErrorFeatureNotPresent
    ...
    vk::PhysicalDeviceDynamicRenderingFeaturesKHR supportedDynamic;
    supportedFeatures2.pNext = &supported11;
    supported11.pNext = &supported12;
    supported12.pNext = &supportedDynamic;
    physicalDevice.getFeatures2(&supportedFeatures2);

    // Filter requested features based on support
    vk::PhysicalDeviceFeatures enabledFeatures = deviceFeatures;
    if (deviceFeatures.samplerAnisotropy && !supportedFeatures2.features.samplerAnisotropy) {
        spdlog::warn("Sampler Anisotropy requested but not supported. Disabling.");
        enabledFeatures.samplerAnisotropy = VK_FALSE;
    }
    if (deviceFeatures.shaderInt64 && !supportedFeatures2.features.shaderInt64) {
        spdlog::warn("ShaderInt64 requested but not supported. Disabling.");
        enabledFeatures.shaderInt64 = VK_FALSE;
    }
    if (deviceFeatures.shaderStorageImageWriteWithoutFormat && !supportedFeatures2.features.shaderStorageImageWriteWithoutFormat) {
        spdlog::warn("ShaderStorageImageWriteWithoutFormat requested but not supported. Disabling.");
        enabledFeatures.shaderStorageImageWriteWithoutFormat = VK_FALSE;
    }

    // Set structure types for Vulkan-HPP
    supported11.sType = vk::StructureType::ePhysicalDeviceVulkan11Features;
    supported12.sType = vk::StructureType::ePhysicalDeviceVulkan12Features;
    supportedDynamic.sType = vk::StructureType::ePhysicalDeviceDynamicRenderingFeatures;

    vk::PhysicalDeviceVulkan11Features enabled11 = deviceFeatures11;
    enabled11.sType = vk::StructureType::ePhysicalDeviceVulkan11Features;
    
    vk::PhysicalDeviceVulkan12Features enabled12 = deviceFeatures12;
    enabled12.sType = vk::StructureType::ePhysicalDeviceVulkan12Features;
    
    if (deviceFeatures12.shaderBufferInt64Atomics && !supported12.shaderBufferInt64Atomics) {
        spdlog::warn("ShaderBufferInt64Atomics requested but not supported. Disabling.");
        enabled12.shaderBufferInt64Atomics = VK_FALSE;
    }
    if (deviceFeatures12.shaderSharedInt64Atomics && !supported12.shaderSharedInt64Atomics) {
        spdlog::warn("ShaderSharedInt64Atomics requested but not supported. Disabling.");
        enabled12.shaderSharedInt64Atomics = VK_FALSE;
    }
    ...
    vk::DeviceCreateInfo createInfo = {
        ...
        (uint32_t) deviceExtensionsCharPtr.size(), deviceExtensionsCharPtr.data(), &enabledFeatures
    };
    
    vk::PhysicalDeviceDynamicRenderingFeaturesKHR dynamicRenderingFeatures {};
    dynamicRenderingFeatures.sType = vk::StructureType::ePhysicalDeviceDynamicRenderingFeatures;
    
    // Only enable dynamic rendering if supported
    if (supportedDynamic.dynamicRendering) {
        dynamicRenderingFeatures.dynamicRendering = VK_TRUE;
    } else {
        spdlog::warn("DynamicRendering requested but not supported. Disabling.");
        dynamicRenderingFeatures.dynamicRendering = VK_FALSE;
    }
    
    // Build pNext chain for logical device creation
    createInfo.pNext = &enabled11;
    enabled11.pNext = &enabled12;
    enabled12.pNext = &dynamicRenderingFeatures;
...
void VulkanContext::setupVma() {
    ...
    // 树莓派 (Broadcom V3D) 上的某些 Vulkan 驱动在某些配置下可能需要这个 flag 来正确处理内存分配
    // 特别是在混合使用共享和独占模式时，确保 VMA 内部能正确处理
